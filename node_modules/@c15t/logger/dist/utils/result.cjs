...
const logResult = (result, customLogger = logger_cjs_namespaceObject.logger, messagePrefix = 'Error:')=>{
    if (result.isErr()) {
...
const logResultAsync = (resultAsync, customLogger = logger_cjs_namespaceObject.logger, messagePrefix = 'Error:')=>resultAsync.mapErr((error)=>{
        customLogger.error(`${messagePrefix} ${error.message}`, error);
        return error;
    });
