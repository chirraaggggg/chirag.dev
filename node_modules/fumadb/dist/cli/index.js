...
function createCli(options) {
  const db = options.db;
  async function selectVersion(defaultValue) {
  ...
  async function inputOutputPath(type, suggestion) {
    const result = await text({
      message: type === "sql" ? "Where to output the SQL migration file?" : "Where to output the generated schema? (it will override the destination)",
  ...
  return {
    async main() {
      ...
      program.name(options.command).description(
        options.description ?? "FumaDB CLI for migrations and schema generation"
      ).version(options.version);
      program.command("migrate:up").description("Migrate to the next schema version").action(async () => {
      ...
      program.command("migrate:down").description("Rollback to the previous schema version").action(async () => {
        const migrator = db.createMigrator();
      ...
      program.command("migrate:to [version]").alias("migrate").description(
        ...
        const result = version === "latest" ? await migrator.migrateToLatest() : await migrator.migrateTo(version);
        await result.execute();
        console.log(`Migrated to version ${version}.`);
      });
      program.command("generate [version]").description(
